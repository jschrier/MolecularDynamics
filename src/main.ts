...
app.innerHTML = `
  <header><h1>Willy P Chem MD <small>browser edition</small></h1><p>Client-side Lennard-Jones molecular dynamics with WebAssembly and WebGL.</p></header>
  ...
  <div class="playback"><button id="previous" disabled>‹ Step</button><button id="play" disabled>Play</button><button id="next" disabled>Step ›</button><label>Speed <select id="speed" disabled><option value="10">Slow</option><option value="30" selected>Normal</option><option value="90">Fast</option></select></label></div><input id="scrubber" type="range" min="0" max="0" value="0" disabled aria-label="Trajectory frame" /></section>
  <section class="panel output"><div class="view-head"><h2>Textual output</h2><div><button id="download-output" disabled>Download output.txt</button><button id="download-average" disabled>Download average.txt</button></div></div><pre id="transcript">The reference-style transcript will appear here.</pre><h3>Instantaneous data</h3><div id="rows" class="rows">No simulation data.</div></section></main>`;
...
worker.onmessage = ({ data }: MessageEvent<WorkerEvent>) => {
  if(data.type==='progress') { progress.value=data.completed; status.textContent=`Simulating… ${(data.completed/data.total*100).toFixed(1)}%`; }
  if(data.type==='error') { status.textContent=`Unable to start: ${data.message}`; start.disabled=false; cancel.disabled=true; }
  if(data.type==='cancelled') { status.textContent='Simulation canceled.'; start.disabled=false; cancel.disabled=true; }
  if(data.type==='complete') { frames=data.frameCount; output=data.output; averages=data.averages; renderer.load(new Float32Array(data.frames),frames,data.boxLength); showFrame(0); playable(true); transcript.textContent=data.transcript+'\n\n'+averages; renderRows(output); status.textContent='Simulation complete.'; progress.value=1000; start.disabled=false; cancel.disabled=true; $<HTMLButtonElement>('#download-output').disabled=false; $<HTMLButtonElement>('#download-average').disabled=false; }
};
form.addEventListener('submit',(event)=>{ event.preventDefault(); const data=new FormData(form); const temperature=Number(data.get('temperature')), density=Number(data.get('density')); title=String(data.get('title')).trim()||'calculation'; const seedText=String(data.get('seed')).trim(); const input:SimulationInput={title,gas:String(data.get('gas')) as SimulationInput['gas'],temperatureKelvin:temperature,densityMolesPerM3:density,...(seedText?{seed:Number(seedText)}:{})}; const problem=validateInput(input); if(problem){ status.textContent=problem; return; } start.disabled=true; cancel.disabled=false; progress.value=0; playable(false); status.textContent='Starting WebAssembly worker…'; worker.postMessage({type:'start',input}); });
cancel.onclick=()=>{ worker.postMessage({type:'cancel'}); status.textContent='Canceling at the next simulation chunk…'; }; reset.onclick=()=>{ worker.postMessage({type:'reset'}); clearTimeout(timer); playing=false; frame=frames=0; playable(false); progress.value=0; status.textContent='Ready.'; transcript.textContent='The reference-style transcript will appear here.'; rows.textContent='No simulation data.'; label.textContent='Run a simulation to view particles.'; };
